...
<#
.SYNOPSIS
    Downloads and installs Adobe-Acrobat-x64-Update using data from Software-Versions.json
    
.DESCRIPTION
    This script downloads and installs Adobe-Acrobat-x64-Update following these steps:
    1. Checks if Adobe-Acrobat-x64-Update is already installed and compares version
    ...
    5. Installs Adobe-Acrobat-x64-Update with specified arguments
...
$SoftwareName = "Adobe-Acrobat-x64-Update"
$JsonUrls = @(
...
try {
    Write-ColorOutput "=== Adobe-Acrobat-x64-Update Installation Script ===" "Cyan"
    if ($DisableInstall) {
    ...
    Write-ColorOutput "Starting Adobe-Acrobat-x64-Update installation process..." "White"
    ...
    foreach ($jsonUrl in $JsonUrls) {
        ...
        while ($retryCount -le $maxRetries -and -not $jsonLoaded) {
            try {
                if ($retryCount -gt 0) {
                    Write-ColorOutput "Retry attempt $retryCount of $maxRetries for: $jsonUrl" "Yellow"
                    Start-Sleep -Seconds ($retryDelay * $retryCount) # Exponential backoff
                } else {
                    Write-ColorOutput "Attempting to load JSON from: $jsonUrl" "White"
                }
                
                $jsonContent = Invoke-RestMethod -Uri $jsonUrl -UseBasicParsing -TimeoutSec 30 -UserAgent "PowerShell-Script/1.0"
                
                if ($jsonContent -and $jsonContent.$SoftwareName) {
                    Write-ColorOutput "Successfully loaded JSON configuration from: $jsonUrl" "Green"
                    $jsonLoaded = $true
                    break
                } else {
                    throw "JSON content is empty or missing software configuration for: $SoftwareName"
                }
            }
            catch {
                $retryCount++
                Write-ColorOutput "Failed to load JSON from $jsonUrl (attempt $retryCount): $($_.Exception.Message)" "Yellow"
                
                if ($retryCount -gt $maxRetries) {
                    Write-ColorOutput "All retry attempts failed for: $jsonUrl" "Red"
                    break
                }
            }
        }
    ...
    if (-not $jsonLoaded) {
        throw "Failed to load JSON configuration from all URLs after retries: $($JsonUrls -join ', ')"
    }
    ...
    Write-ColorOutput "Checking if Adobe-Acrobat-x64-Update is already installed..." "White"
    ...
    if ($installedVersion) {
        ...
        if (-not $needsUpdate) {
            Write-ColorOutput "Adobe-Acrobat-x64-Update is already up to date. No installation needed." "Green"
            return 0
        }
        else {
        ...
        Write-ColorOutput "Adobe-Acrobat-x64-Update not found. Proceeding with installation..." "Yellow"
    }
    ...
    # Handle version conversion for Adobe-Acrobat-x64-Update (remove decimal point)
    ...
    if (-not $downloadSuccess) {
        throw "Failed to download Adobe-Acrobat-x64-Update installer after $($maxRetries + 1) attempts"
    }
    ...
    if (-not [string]::IsNullOrWhiteSpace($softwareConfig.processes)) {
        Write-ColorOutput "Killing specified Adobe-Acrobat-x64-Update processes..." "White"
        if ($DisableInstall) {
    ...
    # Install software
    Write-ColorOutput "Installing Adobe-Acrobat-x64-Update..." "White"
    ...
    while (-not $installSuccess -and $installRetryCount -le $maxInstallRetries) {
        ...
        try {
            if ($DisableInstall) {
                Write-ColorOutput "[DRY-RUN] Would install Adobe-Acrobat-x64-Update with msiexec /p: $downloadPath $($softwareConfig.arguments)" "Cyan"
                $installSuccess = $true
            }
            else {
            ...
            if ($DisableInstall) {
                ...
                Write-ColorOutput "Adobe-Acrobat-x64-Update installation error: $($_.Exception.Message)" "Red"
    ...
    if (-not $installSuccess) {
        Write-ColorOutput "Adobe-Acrobat-x64-Update installation failed after $($maxInstallRetries + 1) attempts. Leaving installer in: $downloadDir" "Red"
        throw "Adobe-Acrobat-x64-Update installation failed"
    }
    
    # Verify software installation
    Write-ColorOutput "Verifying Adobe-Acrobat-x64-Update installation..." "White"
    
    if ($DisableInstall) {
        Write-ColorOutput "[DRY-RUN] Would verify Adobe-Acrobat-x64-Update installation" "Cyan"
        ...
        Write-ColorOutput "=== Adobe-Acrobat-x64-Update Installation Completed Successfully (DRY-RUN) ===" "Green"
        return 0
    }
    else {
        ...
        if ($installationVerified -and $versionMatch) {
            Write-ColorOutput "Adobe-Acrobat-x64-Update installation and version verification successful" "Green"
            ...
            Write-ColorOutput "=== Adobe-Acrobat-x64-Update Installation Completed Successfully ===" "Green"
            return 0
        } else {
            Write-ColorOutput "Adobe-Acrobat-x64-Update installation or version verification failed" "Red"
            Write-ColorOutput "Leaving installer in: $downloadDir" "Yellow"
            throw "Adobe-Acrobat-x64-Update installation or version verification failed"
        }
    }
}
catch {
    Write-ColorOutput "=== Adobe-Acrobat-x64-Update Installation Failed ===" "Red"
