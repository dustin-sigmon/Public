...
<#
.SYNOPSIS
    Uninstalls Adobe Acrobat 2017 from the system
    
.DESCRIPTION
    This script uninstalls Adobe Acrobat 2017 following these steps:
    1. Kills Adobe processes to prevent conflicts
    2. Searches for Adobe Acrobat 2017 installations in registry
...
# Function to check if Adobe Acrobat 2017 is installed
function Test-AdobeAcrobat2017Installed {
    try {
        $Apps = @()
        ...
        # Look for Adobe Acrobat 2017 installation name
        $adobeInstallations = ($apps | Where-Object { $_.DisplayName -like "Adobe Acrobat 2017" })
        
        if ($adobeInstallations) {
            Write-ColorOutput "Found Adobe Acrobat 2017 installations:" "Yellow"
            ...
            Write-ColorOutput "No Adobe Acrobat 2017 installations found." "Green"
            return $null
        }
    }
    catch {
        Write-ColorOutput "Error checking for Adobe Acrobat 2017 installations: $($_.Exception.Message)" "Red"
        return $null
    }
}
...
try {
    Write-ColorOutput "=== Adobe Acrobat 2017 Uninstall Script ===" "Cyan"
    if ($DisableUninstall) {
    ...
    Write-ColorOutput "Starting Adobe Acrobat 2017 uninstall process..." "White"
    
    # Check if Adobe Acrobat 2017 is installed
    Write-ColorOutput "Checking for Adobe Acrobat 2017 installations..." "White"
    $adobeInstallations = Test-AdobeAcrobat2017Installed
    
    if (-not $adobeInstallations) {
        Write-ColorOutput "Adobe Acrobat 2017 is not installed. Nothing to uninstall." "Green"
        return 0
    }
    ...
    Write-ColorOutput "Uninstalling Adobe Acrobat 2017..." "White"
    
    if ($DisableUninstall) {
        Write-ColorOutput "[DRY-RUN] Would uninstall Adobe Acrobat 2017 installations" "Cyan"
        foreach ($installation in $adobeInstallations) {
    ...
    if ($DisableUninstall) {
        ...
        $remainingInstallations = Test-AdobeAcrobat2017Installed
        
        if (-not $remainingInstallations) {
            Write-ColorOutput "Adobe Acrobat 2017 uninstall verification successful - no installations remain." "Green"
        }
        else {
            Write-ColorOutput "Warning: Some Adobe Acrobat 2017 installations may still be present." "Yellow"
    ...
    Write-ColorOutput "=== Adobe Acrobat 2017 Uninstall Completed Successfully ===" "Green"
    return 0
}
catch {
    Write-ColorOutput "=== Adobe Acrobat 2017 Uninstall Failed ===" "Red"
