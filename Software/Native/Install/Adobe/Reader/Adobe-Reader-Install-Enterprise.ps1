...
<#
.SYNOPSIS
    Downloads and installs Adobe-Acrobat-Enterprise-Unified-Installerer using data from Software-Versions.json
    
.DESCRIPTION
    This script downloads and installs Adobe-Acrobat-Enterprise-Unified-Installerer following these steps:
    1. Checks if Adobe-Acrobat-Enterprise-Unified-Installerer is already installed and compares version
    ...
    5. Installs Adobe-Acrobat-Enterprise-Unified-Installerer with specified arguments
...
$SoftwareName = "Adobe-Acrobat-Enterprise-Unified-Installer"
$JsonUrls = @(
...
try {
    Write-ColorOutput "=== Adobe-Acrobat-Enterprise-Unified-Installerer Installation Script ===" "Cyan"
    if ($DisableInstall) {
    ...
    Write-ColorOutput "Starting Adobe-Acrobat-Enterprise-Unified-Installerer installation process..." "White"
    ...
    Write-ColorOutput "Checking if Adobe-Acrobat-Enterprise-Unified-Installerer is already installed..." "White"
    ...
    if ($installedVersion) {
        ...
        if (-not $needsUpdate) {
            Write-ColorOutput "Adobe-Acrobat-Enterprise-Unified-Installerer is already up to date. No installation needed." "Green"
            return 0
        }
        else {
        ...
        Write-ColorOutput "Adobe-Acrobat-Enterprise-Unified-Installerer not found. Proceeding with installation..." "Yellow"
    }
    ...
    # Handle version conversion for Adobe-Acrobat-Enterprise-Unified-Installerer (remove decimal point)
    ...
    foreach ($url in $downloadUrls) {
        $actualUrl = $url -replace '\$\{PublisherName\}', $publisherName -replace '\$\{SoftwareName\}', $SoftwareName -replace '\$\{DownloadFile\}', $downloadFile
    ...
    while (-not $downloadSuccess -and $retryCount -le $maxRetries) {
        ...
        foreach ($url in $downloadUrls) {
            $downloadUrl = $url -replace '\$\{PublisherName\}', $publisherName -replace '\$\{SoftwareName\}', $SoftwareName -replace '\$\{DownloadFile\}', $downloadFile -replace '\$\{SoftwareVersion\}', $softwareConfig.version
    ...
    if (-not $downloadSuccess) {
        throw "Failed to download Adobe-Acrobat-Enterprise-Unified-Installerer installer after $($maxRetries + 1) attempts"
    }
    ...
    if (-not [string]::IsNullOrWhiteSpace($softwareConfig.processes)) {
        Write-ColorOutput "Killing specified Adobe-Acrobat-Enterprise-Unified-Installerer processes..." "White"
        if ($DisableInstall) {
    ...
    # Install software
    Write-ColorOutput "Installing Adobe-Acrobat-Enterprise-Unified-Installerer..." "White"
    ...
    while (-not $installSuccess -and $installRetryCount -le $maxInstallRetries) {
        ...
        try {
            if ($DisableInstall) {
                Write-ColorOutput "[DRY-RUN] Would install Adobe-Acrobat-Enterprise-Unified-Installerer with arguments: $($softwareConfig.arguments)" "Cyan"
                Write-ColorOutput "[DRY-RUN] Would use installer: $downloadPath" "Cyan"
                $installSuccess = $true
            }
            else {
                $processArgs = @{
                    FilePath     = $downloadPath
                    ArgumentList = $softwareConfig.arguments.Split(' ')
                    Wait         = $true
                    PassThru     = $true
                }
                ...
                if ($installProcess.ExitCode -eq 0) {
                    Write-ColorOutput "Adobe-Acrobat-Enterprise-Unified-Installerer installation completed successfully" "Green"
                    $installSuccess = $true
                }
                else {
                    Write-ColorOutput "Adobe-Acrobat-Enterprise-Unified-Installerer installation failed with exit code: $($installProcess.ExitCode)" "Red"
            ...
            if ($DisableInstall) {
                ...
                Write-ColorOutput "Adobe-Acrobat-Enterprise-Unified-Installerer installation error: $($_.Exception.Message)" "Red"
    ...
    if (-not $installSuccess) {
        Write-ColorOutput "Adobe-Acrobat-Enterprise-Unified-Installerer installation failed after $($maxInstallRetries + 1) attempts. Leaving installer in: $downloadDir" "Red"
        throw "Adobe-Acrobat-Enterprise-Unified-Installerer installation failed"
    }
    
    # Verify software installation
    Write-ColorOutput "Verifying Adobe-Acrobat-Enterprise-Unified-Installerer installation..." "White"
    
    if ($DisableInstall) {
        Write-ColorOutput "[DRY-RUN] Would verify Adobe-Acrobat-Enterprise-Unified-Installerer installation" "Cyan"
        ...
        Write-ColorOutput "=== Adobe-Acrobat-Enterprise-Unified-Installerer Installation Completed Successfully (DRY-RUN) ===" "Green"
        return 0
    }
    else {
        ...
        if ($installationVerified) {
            Write-ColorOutput "Adobe-Acrobat-Enterprise-Unified-Installerer installation verification successful" "Green"
            ...
            Write-ColorOutput "Adobe-Acrobat-Enterprise-Unified-Installerer installation verification failed" "Red"
            Write-ColorOutput "Leaving installer in: $downloadDir" "Yellow"
            throw "Adobe-Acrobat-Enterprise-Unified-Installerer installation verification failed"
        }
    }
}
catch {
    Write-ColorOutput "=== Adobe-Acrobat-Enterprise-Unified-Installerer Installation Failed ===" "Red"
