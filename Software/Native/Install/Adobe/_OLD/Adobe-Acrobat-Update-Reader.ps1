...
<#
.SYNOPSIS
    Downloads and installs Adobe-Acrobat-Update-Reader using data from Software-Versions.json
    
.DESCRIPTION
    This script downloads and installs Adobe-Acrobat-Update-Reader following these steps:
    1. Checks if Adobe-Acrobat-Update-Reader is already installed and compares version
    ...
    5. Installs Adobe-Acrobat-Update-Reader with specified arguments
...
$SoftwareName = "Adobe-Acrobat-Update-Reader"
$JsonUrls = @(
...
try {
    Write-ColorOutput "=== Adobe-Acrobat-Update-Reader Installation Script ===" "Cyan"
    if ($DisableInstall) {
    ...
    Write-ColorOutput "Starting Adobe-Acrobat-Update-Reader installation process..." "White"
    ...
    Write-ColorOutput "Checking if Adobe-Acrobat-Update-Reader is already installed..." "White"
    ...
    if ($installedVersion) {
        ...
        if (-not $needsUpdate) {
            Write-ColorOutput "Adobe-Acrobat-Update-Reader is already up to date. No installation needed." "Green"
            return 0
        }
        else {
        ...
        Write-ColorOutput "Adobe-Acrobat-Update-Reader not found. Proceeding with installation..." "Yellow"
    }
    ...
    # Handle version conversion for Adobe-Acrobat-Update-Reader (remove decimal point)
    ...
    while (-not $downloadSuccess -and $retryCount -le $maxRetries) {
        ...
        foreach ($url in $downloadUrls) {
            $downloadUrl = $url -replace '\$\{PublisherName\}', $publisherName -replace '\$\{SoftwareName\}', $SoftwareName -replace '\$\{DownloadFile\}', $downloadFile
    ...
    if (-not $downloadSuccess) {
        throw "Failed to download Adobe-Acrobat-Update-Reader installer after $($maxRetries + 1) attempts"
    }
    ...
    if (-not [string]::IsNullOrWhiteSpace($softwareConfig.processes)) {
        Write-ColorOutput "Killing specified Adobe-Acrobat-Update-Reader processes..." "White"
        if ($DisableInstall) {
    ...
    # Install software
    Write-ColorOutput "Installing Adobe-Acrobat-Update-Reader..." "White"
    ...
    while (-not $installSuccess -and $installRetryCount -le $maxInstallRetries) {
        ...
        try {
            if ($DisableInstall) {
                Write-ColorOutput "[DRY-RUN] Would install Adobe-Acrobat-Update-Reader with msiexec /p: $downloadPath $($softwareConfig.arguments)" "Cyan"
                $installSuccess = $true
            }
            else {
            ...
            if ($DisableInstall) {
                ...
                Write-ColorOutput "Adobe-Acrobat-Update-Reader installation error: $($_.Exception.Message)" "Red"
    ...
    if (-not $installSuccess) {
        Write-ColorOutput "Adobe-Acrobat-Update-Reader installation failed after $($maxInstallRetries + 1) attempts. Leaving installer in: $downloadDir" "Red"
        throw "Adobe-Acrobat-Update-Reader installation failed"
    }
    
    # Verify software installation
    Write-ColorOutput "Verifying Adobe-Acrobat-Update-Reader installation..." "White"
    
    if ($DisableInstall) {
        Write-ColorOutput "[DRY-RUN] Would verify Adobe-Acrobat-Update-Reader installation" "Cyan"
        ...
        Write-ColorOutput "=== Adobe-Acrobat-Update-Reader Installation Completed Successfully (DRY-RUN) ===" "Green"
        return 0
    }
    else {
        ...
        if ($installationVerified) {
            Write-ColorOutput "Adobe-Acrobat-Update-Reader installation verification successful" "Green"
            ...
            Write-ColorOutput "=== Adobe-Acrobat-Update-Reader Installation Completed Successfully ===" "Green"
            return 0
        }
        else {
            Write-ColorOutput "Adobe-Acrobat-Update-Reader installation verification failed" "Red"
            Write-ColorOutput "Leaving installer in: $downloadDir" "Yellow"
            throw "Adobe-Acrobat-Update-Reader installation verification failed"
        }
    }
}
catch {
    Write-ColorOutput "=== Adobe-Acrobat-Update-Reader Installation Failed ===" "Red"
