...
<#
.SYNOPSIS
    Downloads and installs Adobe-Reader-x64-ENU using data from Software-Versions.json
    
.DESCRIPTION
    This script downloads and installs Adobe-Reader-x64-ENU following these steps:
    1. Checks if Adobe-Reader-x64-ENU is already installed and compares version
    ...
    5. Installs Adobe-Reader-x64-ENU with specified arguments
...
$SoftwareName = "Adobe-Reader-x64-ENU"
$JsonUrls = @(
...
try {
    Write-ColorOutput "=== Adobe-Reader-x64-ENU Installation Script ===" "Cyan"
    if ($DisableInstall) {
    ...
    Write-ColorOutput "Starting Adobe-Reader-x64-ENU installation process..." "White"
    ...
    Write-ColorOutput "Checking if Adobe-Reader-x64-ENU is already installed..." "White"
    ...
    if ($installedVersion) {
        ...
        if (-not $needsUpdate) {
            Write-ColorOutput "Adobe-Reader-x64-ENU is already up to date. No installation needed." "Green"
            return 0
        }
        else {
        ...
        Write-ColorOutput "Adobe-Reader-x64-ENU not found. Proceeding with installation..." "Yellow"
    }
    ...
    # Handle version conversion for Adobe-Reader-x64-ENU (remove decimal point)
    ...
    if (-not $downloadSuccess) {
        throw "Failed to download Adobe-Reader-x64-ENU installer after $($maxRetries + 1) attempts"
    }
    ...
    if (-not [string]::IsNullOrWhiteSpace($softwareConfig.processes)) {
        Write-ColorOutput "Killing specified Adobe-Reader-x64-ENU processes..." "White"
        if ($DisableInstall) {
    ...
    # Install software
    Write-ColorOutput "Installing Adobe-Reader-x64-ENU..." "White"
    ...
    while (-not $installSuccess -and $installRetryCount -le $maxInstallRetries) {
        ...
        try {
            if ($DisableInstall) {
                Write-ColorOutput "[DRY-RUN] Would install Adobe-Reader-x64-ENU with msiexec /p: $downloadPath $($softwareConfig.arguments)" "Cyan"
                $installSuccess = $true
            }
            else {
            ...
            if ($DisableInstall) {
                ...
                Write-ColorOutput "Adobe-Reader-x64-ENU installation error: $($_.Exception.Message)" "Red"
    ...
    if (-not $installSuccess) {
        Write-ColorOutput "Adobe-Reader-x64-ENU installation failed after $($maxInstallRetries + 1) attempts. Leaving installer in: $downloadDir" "Red"
        throw "Adobe-Reader-x64-ENU installation failed"
    }
    
    # Verify software installation
    Write-ColorOutput "Verifying Adobe-Reader-x64-ENU installation..." "White"
    
    if ($DisableInstall) {
        Write-ColorOutput "[DRY-RUN] Would verify Adobe-Reader-x64-ENU installation" "Cyan"
        ...
        Write-ColorOutput "=== Adobe-Reader-x64-ENU Installation Completed Successfully (DRY-RUN) ===" "Green"
        return 0
    }
    else {
        ...
        if ($installationVerified) {
            Write-ColorOutput "Adobe-Reader-x64-ENU installation verification successful" "Green"
            ...
            Write-ColorOutput "=== Adobe-Reader-x64-ENU Installation Completed Successfully ===" "Green"
            return 0
        }
        else {
            Write-ColorOutput "Adobe-Reader-x64-ENU installation verification failed" "Red"
            Write-ColorOutput "Leaving installer in: $downloadDir" "Yellow"
            throw "Adobe-Reader-x64-ENU installation verification failed"
        }
    }
}
catch {
    Write-ColorOutput "=== Adobe-Reader-x64-ENU Installation Failed ===" "Red"
