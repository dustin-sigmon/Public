...
<#
.SYNOPSIS
    Downloads and installs Adobe-Reader-x86-MUI using data from Software-Versions.json
    
.DESCRIPTION
    This script downloads and installs Adobe-Reader-x86-MUI following these steps:
    1. Checks if Adobe-Reader-x86-MUI is already installed and compares version
    ...
    5. Installs Adobe-Reader-x86-MUI with specified arguments
...
$SoftwareName = "Adobe-Reader-x86-MUI"
$JsonUrls = @(
...
try {
    Write-ColorOutput "=== Adobe-Reader-x86-MUI Installation Script ===" "Cyan"
    if ($DisableInstall) {
    ...
    Write-ColorOutput "Starting Adobe-Reader-x86-MUI installation process..." "White"
    ...
    Write-ColorOutput "Checking if Adobe-Reader-x86-MUI is already installed..." "White"
    ...
    if ($installedVersion) {
        ...
        if (-not $needsUpdate) {
            Write-ColorOutput "Adobe-Reader-x86-MUI is already up to date. No installation needed." "Green"
            return 0
        }
        else {
        ...
        Write-ColorOutput "Adobe-Reader-x86-MUI not found. Proceeding with installation..." "Yellow"
    }
    ...
    # Handle version conversion for Adobe-Reader-x86-MUI (remove decimal point)
    ...
    if (-not $downloadSuccess) {
        throw "Failed to download Adobe-Reader-x86-MUI installer after $($maxRetries + 1) attempts"
    }
    ...
    if (-not [string]::IsNullOrWhiteSpace($softwareConfig.processes)) {
        Write-ColorOutput "Killing specified Adobe-Reader-x86-MUI processes..." "White"
        if ($DisableInstall) {
    ...
    # Install software
    Write-ColorOutput "Installing Adobe-Reader-x86-MUI..." "White"
    ...
    while (-not $installSuccess -and $installRetryCount -le $maxInstallRetries) {
        ...
        try {
            if ($DisableInstall) {
                Write-ColorOutput "[DRY-RUN] Would install Adobe-Reader-x86-MUI with msiexec /p: $downloadPath $($softwareConfig.arguments)" "Cyan"
                $installSuccess = $true
            }
            else {
            ...
            if ($DisableInstall) {
                ...
                Write-ColorOutput "Adobe-Reader-x86-MUI installation error: $($_.Exception.Message)" "Red"
    ...
    if (-not $installSuccess) {
        Write-ColorOutput "Adobe-Reader-x86-MUI installation failed after $($maxInstallRetries + 1) attempts. Leaving installer in: $downloadDir" "Red"
        throw "Adobe-Reader-x86-MUI installation failed"
    }
    
    # Verify software installation
    Write-ColorOutput "Verifying Adobe-Reader-x86-MUI installation..." "White"
    
    if ($DisableInstall) {
        Write-ColorOutput "[DRY-RUN] Would verify Adobe-Reader-x86-MUI installation" "Cyan"
        ...
        Write-ColorOutput "=== Adobe-Reader-x86-MUI Installation Completed Successfully (DRY-RUN) ===" "Green"
        return 0
    }
    else {
        ...
        if ($installationVerified) {
            Write-ColorOutput "Adobe-Reader-x86-MUI installation verification successful" "Green"
            ...
            Write-ColorOutput "=== Adobe-Reader-x86-MUI Installation Completed Successfully ===" "Green"
            return 0
        }
        else {
            Write-ColorOutput "Adobe-Reader-x86-MUI installation verification failed" "Red"
            Write-ColorOutput "Leaving installer in: $downloadDir" "Yellow"
            throw "Adobe-Reader-x86-MUI installation verification failed"
        }
    }
}
catch {
    Write-ColorOutput "=== Adobe-Reader-x86-MUI Installation Failed ===" "Red"
