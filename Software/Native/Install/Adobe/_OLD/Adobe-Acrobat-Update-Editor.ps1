...
<#
.SYNOPSIS
    Downloads and installs Adobe-Acrobat-Update-Editor using data from Software-Versions.json
    
.DESCRIPTION
    This script downloads and installs Adobe-Acrobat-Update-Editor following these steps:
    1. Checks if Adobe-Acrobat-Update-Editor is already installed and compares version
    ...
    5. Installs Adobe-Acrobat-Update-Editor with specified arguments
...
$SoftwareName = "Adobe-Acrobat-Update-Editor"
$JsonUrls = @(
...
try {
    Write-ColorOutput "=== Adobe-Acrobat-Update-Editor Installation Script ===" "Cyan"
    if ($DisableInstall) {
    ...
    Write-ColorOutput "Starting Adobe-Acrobat-Update-Editor installation process..." "White"
    ...
    Write-ColorOutput "Checking if Adobe-Acrobat-Update-Editor is already installed..." "White"
    ...
    if ($installedVersion) {
        ...
        if (-not $needsUpdate) {
            Write-ColorOutput "Adobe-Acrobat-Update-Editor is already up to date. No installation needed." "Green"
            return 0
        }
        else {
        ...
        Write-ColorOutput "Adobe-Acrobat-Update-Editor not found. Proceeding with installation..." "Yellow"
    }
    ...
    # Handle version conversion for Adobe-Acrobat-Update-Editor (remove decimal point)
    ...
    if (-not $downloadSuccess) {
        throw "Failed to download Adobe-Acrobat-Update-Editor installer after $($maxRetries + 1) attempts"
    }
    ...
    if (-not [string]::IsNullOrWhiteSpace($softwareConfig.processes)) {
        Write-ColorOutput "Killing specified Adobe-Acrobat-Update-Editor processes..." "White"
        if ($DisableInstall) {
    ...
    # Install software
    Write-ColorOutput "Installing Adobe-Acrobat-Update-Editor..." "White"
    ...
    while (-not $installSuccess -and $installRetryCount -le $maxInstallRetries) {
        ...
        try {
            if ($DisableInstall) {
                Write-ColorOutput "[DRY-RUN] Would install Adobe-Acrobat-Update-Editor with msiexec /p: $downloadPath $($softwareConfig.arguments)" "Cyan"
                $installSuccess = $true
            }
            else {
            ...
            if ($DisableInstall) {
                ...
                Write-ColorOutput "Adobe-Acrobat-Update-Editor installation error: $($_.Exception.Message)" "Red"
    ...
    if (-not $installSuccess) {
        Write-ColorOutput "Adobe-Acrobat-Update-Editor installation failed after $($maxInstallRetries + 1) attempts. Leaving installer in: $downloadDir" "Red"
        throw "Adobe-Acrobat-Update-Editor installation failed"
    }
    
    # Verify software installation
    Write-ColorOutput "Verifying Adobe-Acrobat-Update-Editor installation..." "White"
    
    if ($DisableInstall) {
        Write-ColorOutput "[DRY-RUN] Would verify Adobe-Acrobat-Update-Editor installation" "Cyan"
        ...
        Write-ColorOutput "=== Adobe-Acrobat-Update-Editor Installation Completed Successfully (DRY-RUN) ===" "Green"
        return 0
    }
    else {
        ...
        if ($installationVerified) {
            Write-ColorOutput "Adobe-Acrobat-Update-Editor installation verification successful" "Green"
            ...
            Write-ColorOutput "=== Adobe-Acrobat-Update-Editor Installation Completed Successfully ===" "Green"
            return 0
        }
        else {
            Write-ColorOutput "Adobe-Acrobat-Update-Editor installation verification failed" "Red"
            Write-ColorOutput "Leaving installer in: $downloadDir" "Yellow"
            throw "Adobe-Acrobat-Update-Editor installation verification failed"
        }
    }
}
catch {
    Write-ColorOutput "=== Adobe-Acrobat-Update-Editor Installation Failed ===" "Red"
