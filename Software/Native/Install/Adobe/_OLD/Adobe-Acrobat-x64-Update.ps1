...
<#
.SYNOPSIS
    Downloads and installs Adobe-Acrobat-x64 using data from Software-Versions.json
    
.DESCRIPTION
    This script downloads and installs Adobe-Acrobat-x64 following these steps:
    1. Checks if Adobe-Acrobat-x64 is already installed and compares version
    ...
    5. Installs Adobe-Acrobat-x64 with specified arguments
...
$SoftwareName = "Adobe-Acrobat-x64"
$JsonUrls = @(
...
try {
    Write-ColorOutput "=== Adobe-Acrobat-x64 Installation Script ===" "Cyan"
    if ($DisableInstall) {
    ...
    Write-ColorOutput "Starting Adobe-Acrobat-x64 installation process..." "White"
    ...
    Write-ColorOutput "Checking if Adobe-Acrobat-x64 is already installed..." "White"
    ...
    if ($installedVersion) {
        ...
        if (-not $needsUpdate) {
            Write-ColorOutput "Adobe-Acrobat-x64 is already up to date. No installation needed." "Green"
            return 0
        }
        else {
        ...
        Write-ColorOutput "Adobe-Acrobat-x64 not found. Proceeding with installation..." "Yellow"
    }
    ...
    # Handle version conversion for Adobe-Acrobat-x64 (remove decimal point)
    ...
    if (-not $downloadSuccess) {
        throw "Failed to download Adobe-Acrobat-x64 installer after $($maxRetries + 1) attempts"
    }
    ...
    if (-not [string]::IsNullOrWhiteSpace($softwareConfig.processes)) {
        Write-ColorOutput "Killing specified Adobe-Acrobat-x64 processes..." "White"
        if ($DisableInstall) {
    ...
    # Install software
    Write-ColorOutput "Installing Adobe-Acrobat-x64..." "White"
    ...
    while (-not $installSuccess -and $installRetryCount -le $maxInstallRetries) {
        ...
        try {
            if ($DisableInstall) {
                Write-ColorOutput "[DRY-RUN] Would install Adobe-Acrobat-x64 with msiexec /p: $downloadPath $($softwareConfig.arguments)" "Cyan"
                $installSuccess = $true
            }
            else {
            ...
            if ($DisableInstall) {
                ...
                Write-ColorOutput "Adobe-Acrobat-x64 installation error: $($_.Exception.Message)" "Red"
    ...
    if (-not $installSuccess) {
        Write-ColorOutput "Adobe-Acrobat-x64 installation failed after $($maxInstallRetries + 1) attempts. Leaving installer in: $downloadDir" "Red"
        throw "Adobe-Acrobat-x64 installation failed"
    }
    
    # Verify software installation
    Write-ColorOutput "Verifying Adobe-Acrobat-x64 installation..." "White"
    
    if ($DisableInstall) {
        Write-ColorOutput "[DRY-RUN] Would verify Adobe-Acrobat-x64 installation" "Cyan"
        ...
        Write-ColorOutput "=== Adobe-Acrobat-x64 Installation Completed Successfully (DRY-RUN) ===" "Green"
        return 0
    }
    else {
        ...
        if ($installationVerified) {
            Write-ColorOutput "Adobe-Acrobat-x64 installation verification successful" "Green"
            ...
            Write-ColorOutput "=== Adobe-Acrobat-x64 Installation Completed Successfully ===" "Green"
            return 0
        }
        else {
            Write-ColorOutput "Adobe-Acrobat-x64 installation verification failed" "Red"
            Write-ColorOutput "Leaving installer in: $downloadDir" "Yellow"
            throw "Adobe-Acrobat-x64 installation verification failed"
        }
    }
}
catch {
    Write-ColorOutput "=== Adobe-Acrobat-x64 Installation Failed ===" "Red"
