...
<#
.SYNOPSIS
    Downloads and installs Adobe-Acrobat-Enterprise-Installer using data from Software-Versions.json
    
.DESCRIPTION
    This script downloads and installs Adobe-Acrobat-Enterprise-Installer following these steps:
    1. Checks if Adobe-Acrobat-Enterprise-Installer is already installed and compares version
    ...
    5. Installs Adobe-Acrobat-Enterprise-Installer with specified arguments
...
$SoftwareName = "Adobe-Acrobat-Enterprise-Installer"
$JsonUrls = @(
...
try {
    Write-ColorOutput "=== Adobe-Acrobat-Enterprise-Installer Installation Script ===" "Cyan"
    if ($DisableInstall) {
    ...
    Write-ColorOutput "Starting Adobe-Acrobat-Enterprise-Installer installation process..." "White"
    ...
    Write-ColorOutput "Checking if Adobe-Acrobat-Enterprise-Installer is already installed..." "White"
    ...
    if ($installedVersion) {
        ...
        if (-not $needsUpdate) {
            Write-ColorOutput "Adobe-Acrobat-Enterprise-Installer is already up to date. No installation needed." "Green"
            return 0
        }
        else {
        ...
        Write-ColorOutput "Adobe-Acrobat-Enterprise-Installer not found. Proceeding with installation..." "Yellow"
    }
    ...
    # Handle version conversion for Adobe-Acrobat-Enterprise-Installer (remove decimal point)
    ...
    if (-not $downloadSuccess) {
        throw "Failed to download Adobe-Acrobat-Enterprise-Installer installer after $($maxRetries + 1) attempts"
    }
    ...
    if (-not [string]::IsNullOrWhiteSpace($softwareConfig.processes)) {
        Write-ColorOutput "Killing specified Adobe-Acrobat-Enterprise-Installer processes..." "White"
        if ($DisableInstall) {
    ...
    # Install software
    Write-ColorOutput "Installing Adobe-Acrobat-Enterprise-Installer..." "White"
    ...
    while (-not $installSuccess -and $installRetryCount -le $maxInstallRetries) {
        ...
        try {
            if ($DisableInstall) {
                Write-ColorOutput "[DRY-RUN] Would install Adobe-Acrobat-Enterprise-Installer with arguments: $($softwareConfig.arguments)" "Cyan"
                ...
                if ($installProcess.ExitCode -eq 0) {
                    Write-ColorOutput "Adobe-Acrobat-Enterprise-Installer installation completed successfully" "Green"
                    $installSuccess = $true
                }
                else {
                    Write-ColorOutput "Adobe-Acrobat-Enterprise-Installer installation failed with exit code: $($installProcess.ExitCode)" "Red"
            ...
            if ($DisableInstall) {
                ...
                Write-ColorOutput "Adobe-Acrobat-Enterprise-Installer installation error: $($_.Exception.Message)" "Red"
    ...
    if (-not $installSuccess) {
        Write-ColorOutput "Adobe-Acrobat-Enterprise-Installer installation failed after $($maxInstallRetries + 1) attempts. Leaving installer in: $downloadDir" "Red"
        throw "Adobe-Acrobat-Enterprise-Installer installation failed"
    }
    
    # Verify software installation
    Write-ColorOutput "Verifying Adobe-Acrobat-Enterprise-Installer installation..." "White"
    
    if ($DisableInstall) {
        Write-ColorOutput "[DRY-RUN] Would verify Adobe-Acrobat-Enterprise-Installer installation" "Cyan"
        ...
        Write-ColorOutput "=== Adobe-Acrobat-Enterprise-Installer Installation Completed Successfully (DRY-RUN) ===" "Green"
        return 0
    }
    else {
        ...
        if ($installationVerified) {
            Write-ColorOutput "Adobe-Acrobat-Enterprise-Installer installation verification successful" "Green"
            ...
            Write-ColorOutput "Adobe-Acrobat-Enterprise-Installer installation verification failed" "Red"
            Write-ColorOutput "Leaving installer in: $downloadDir" "Yellow"
            throw "Adobe-Acrobat-Enterprise-Installer installation verification failed"
        }
    }
}
catch {
    Write-ColorOutput "=== Adobe-Acrobat-Enterprise-Installer Installation Failed ===" "Red"
