...
$SoftwareName = "FortiClient-VPN-7.0"
$JsonUrls = @(
...
try {
    Write-ColorOutput "=== FortiClient-VPN-7.0 Installation Script ===" "Cyan"
    if ($DisableInstall) {
    ...
    Write-ColorOutput "Starting FortiClient-VPN-7.0 installation process..." "White"
    ...
    Write-ColorOutput "Checking if FortiClient-VPN-7.0 is already installed..." "White"
    ...
    if ($installedVersion) {
        ...
        if (-not $needsUpdate) {
            Write-ColorOutput "FortiClient-VPN-7.0 is already up to date. No installation needed." "Green"
            return 0
        }
        else {
        ...
        Write-ColorOutput "FortiClient-VPN-7.0 not found. Proceeding with installation..." "Yellow"
    }
    ...
    # Handle version conversion for FortiClient-VPN-7.0 (remove decimal point)
    ...
    if (-not $downloadSuccess) {
        throw "Failed to download FortiClient-VPN-7.0 installer after $($maxRetries + 1) attempts"
    }
    ...
    if (-not [string]::IsNullOrWhiteSpace($softwareConfig.processes)) {
        Write-ColorOutput "Killing specified FortiClient-VPN-7.0 processes..." "White"
        if ($DisableInstall) {
    ...
    # Install software
    Write-ColorOutput "Installing FortiClient-VPN-7.0..." "White"
    ...
    while (-not $installSuccess -and $installRetryCount -le $maxInstallRetries) {
        ...
        try {
            if ($DisableInstall) {
                Write-ColorOutput "[DRY-RUN] Would install FortiClient-VPN-7.0 with arguments: $($softwareConfig.arguments)" "Cyan"
                ...
                if ($installProcess.ExitCode -eq 0) {
                    Write-ColorOutput "FortiClient-VPN-7.0 installation completed successfully" "Green"
                    $installSuccess = $true
                }
                else {
                    Write-ColorOutput "FortiClient-VPN-7.0 installation failed with exit code: $($installProcess.ExitCode)" "Red"
            ...
            if ($DisableInstall) {
                ...
                Write-ColorOutput "FortiClient-VPN-7.0 installation error: $($_.Exception.Message)" "Red"
    ...
    if (-not $installSuccess) {
        Write-ColorOutput "FortiClient-VPN-7.0 installation failed after $($maxInstallRetries + 1) attempts. Leaving installer in: $downloadDir" "Red"
        throw "FortiClient-VPN-7.0 installation failed"
    }
    
    # Verify software installation
    Write-ColorOutput "Verifying FortiClient-VPN-7.0 installation..." "White"
    
    if ($DisableInstall) {
        Write-ColorOutput "[DRY-RUN] Would verify FortiClient-VPN-7.0 installation" "Cyan"
        ...
        Write-ColorOutput "=== FortiClient-VPN-7.0 Installation Completed Successfully (DRY-RUN) ===" "Green"
        return 0
    }
    else {
        ...
        if ($installationVerified) {
            Write-ColorOutput "FortiClient-VPN-7.0 installation verification successful" "Green"
            ...
            if (-not $DisableCleanup) {
                ...
                try {
                    Remove-Item -Path $downloadDir -Recurse -Force
                    shutdown /r /t 120 /c "FortiClient VPN install/update completed. System will reboot in 2 minutes."
            ...
            Write-ColorOutput "FortiClient-VPN-7.0 installation verification failed" "Red"
            Write-ColorOutput "Leaving installer in: $downloadDir" "Yellow"
            throw "FortiClient-VPN-7.0 installation verification failed"
        }
    }
}
catch {
    Write-ColorOutput "=== FortiClient-VPN-7.0 Installation Failed ===" "Red"
