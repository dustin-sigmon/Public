...
$SoftwareName = "FortiClient-VPN-7.4"
$JsonUrls = @(
...
try {
    Write-ColorOutput "=== FortiClient-VPN-7.4 Installation Script ===" "Cyan"
    if ($DisableInstall) {
    ...
    Write-ColorOutput "Starting FortiClient-VPN-7.4 installation process..." "White"
    ...
    Write-ColorOutput "Checking if FortiClient-VPN-7.4 is already installed..." "White"
    ...
    if ($installedVersion) {
        ...
        if (-not $needsUpdate) {
            Write-ColorOutput "FortiClient-VPN-7.4 is already up to date. No installation needed." "Green"
            return 0
        }
        else {
        ...
        Write-ColorOutput "FortiClient-VPN-7.4 not found. Proceeding with installation..." "Yellow"
    }
    ...
    # Handle version conversion for FortiClient-VPN-7.4 (remove decimal point)
    ...
    if (-not $downloadSuccess) {
        throw "Failed to download FortiClient-VPN-7.4 installer after $($maxRetries + 1) attempts"
    }
    ...
    if (-not [string]::IsNullOrWhiteSpace($softwareConfig.processes)) {
        Write-ColorOutput "Killing specified FortiClient-VPN-7.4 processes..." "White"
        if ($DisableInstall) {
    ...
    # Install software
    Write-ColorOutput "Installing FortiClient-VPN-7.4..." "White"
    ...
    while (-not $installSuccess -and $installRetryCount -le $maxInstallRetries) {
        ...
        try {
            if ($DisableInstall) {
                Write-ColorOutput "[DRY-RUN] Would install FortiClient-VPN-7.4 with arguments: $($softwareConfig.arguments)" "Cyan"
                ...
                if ($installProcess.ExitCode -eq 0) {
                    Write-ColorOutput "FortiClient-VPN-7.4 installation completed successfully" "Green"
                    $installSuccess = $true
                }
                else {
                    Write-ColorOutput "FortiClient-VPN-7.4 installation failed with exit code: $($installProcess.ExitCode)" "Red"
            ...
            if ($DisableInstall) {
                ...
                Write-ColorOutput "FortiClient-VPN-7.4 installation error: $($_.Exception.Message)" "Red"
    ...
    if (-not $installSuccess) {
        Write-ColorOutput "FortiClient-VPN-7.4 installation failed after $($maxInstallRetries + 1) attempts. Leaving installer in: $downloadDir" "Red"
        throw "FortiClient-VPN-7.4 installation failed"
    }
    
    # Verify software installation
    Write-ColorOutput "Verifying FortiClient-VPN-7.4 installation..." "White"
    
    if ($DisableInstall) {
        Write-ColorOutput "[DRY-RUN] Would verify FortiClient-VPN-7.4 installation" "Cyan"
        ...
        if (-not $DisableCleanup) {
            ...
            try {
                ...
                iex (New-Object Net.WebClient).DownloadString('https://source-east-scripts.s3.us-east-va.io.cloud.ovh.us/_Software/FortiClient/FortiClient-VPN-7.0-Profiles.ps1')
        ...
        Write-ColorOutput "=== FortiClient-VPN-7.4 Installation Completed Successfully (DRY-RUN) ===" "Green"
        return 0
    }
    else {
        ...
        if ($installationVerified) {
            Write-ColorOutput "FortiClient-VPN-7.4 installation verification successful" "Green"
            ...
            Write-ColorOutput "FortiClient-VPN-7.4 installation verification failed" "Red"
            Write-ColorOutput "Leaving installer in: $downloadDir" "Yellow"
            throw "FortiClient-VPN-7.4 installation verification failed"
        }
    }
}
catch {
    Write-ColorOutput "=== FortiClient-VPN-7.4 Installation Failed ===" "Red"
