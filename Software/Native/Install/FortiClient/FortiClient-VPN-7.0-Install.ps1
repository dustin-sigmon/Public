    ...
    while (-not $installSuccess -and $installRetryCount -le $maxInstallRetries) {
        ...
        try {
            if ($DisableInstall) {
                ...
                if ($installVerified) {
                    Write-ColorOutput "FortiClient-VPN-7.0 installation verified in registry" "Green"
                    $installSuccess = $true
                }
                else {
                    Write-ColorOutput "FortiClient-VPN-7.0 installation not found in registry after install attempt" "Red"
