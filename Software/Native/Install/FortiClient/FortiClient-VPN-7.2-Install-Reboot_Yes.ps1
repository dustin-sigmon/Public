...
<#
.SYNOPSIS
    Downloads and installs FortiClient-VPN-7.2 using data from Software-Versions.json
    
.DESCRIPTION
    This script downloads and installs FortiClient-VPN-7.2 following these steps:
    1. Checks if FortiClient-VPN-7.2 is already installed and compares version
    ...
    5. Installs FortiClient-VPN-7.2 with specified arguments
...
$SoftwareName = "FortiClient-VPN-7.2"
$JsonUrls = @(
...
try {
    Write-ColorOutput "=== FortiClient-VPN-7.2 Installation Script ===" "Cyan"
    if ($DisableInstall) {
    ...
    Write-ColorOutput "Starting FortiClient-VPN-7.2 installation process..." "White"
    ...
    Write-ColorOutput "Checking if FortiClient-VPN-7.2 is already installed..." "White"
    ...
    if ($installedVersion) {
        ...
        if (-not $needsUpdate) {
            Write-ColorOutput "FortiClient-VPN-7.2 is already up to date. No installation needed." "Green"
            return 0
        }
        else {
        ...
        Write-ColorOutput "FortiClient-VPN-7.2 not found. Proceeding with installation..." "Yellow"
    }
    ...
    # Handle version conversion for FortiClient-VPN-7.2 (remove decimal point)
    ...
    if (-not $downloadSuccess) {
        throw "Failed to download FortiClient-VPN-7.2 installer after $($maxRetries + 1) attempts"
    }
    ...
    if (-not [string]::IsNullOrWhiteSpace($softwareConfig.processes)) {
        Write-ColorOutput "Killing specified FortiClient-VPN-7.2 processes..." "White"
        if ($DisableInstall) {
    ...
    # Install software
    Write-ColorOutput "Installing FortiClient-VPN-7.2..." "White"
    ...
    while (-not $installSuccess -and $installRetryCount -le $maxInstallRetries) {
        ...
        try {
            if ($DisableInstall) {
                Write-ColorOutput "[DRY-RUN] Would install FortiClient-VPN-7.2 with arguments: $($softwareConfig.arguments)" "Cyan"
                ...
                $processArgs = @{
                    ...
                    ArgumentList = $softwareConfig.arguments
                    Wait         = $true
                }
                
                Start-Process @processArgs
                
                # After install, check registry for success
                $installVerified = Test-SoftwareInstallationSuccess -RegistryPattern $softwareConfig.registry
                if ($installVerified) {
                    Write-ColorOutput "FortiClient-VPN-7.2 installation verified in registry" "Green"
                    $installSuccess = $true
                }
                else {
                    Write-ColorOutput "FortiClient-VPN-7.2 installation not found in registry after install attempt" "Red"
            ...
            if ($DisableInstall) {
                ...
                Write-ColorOutput "FortiClient-VPN-7.2 installation error: $($_.Exception.Message)" "Red"
    ...
    if (-not $installSuccess) {
        Write-ColorOutput "FortiClient-VPN-7.2 installation failed after $($maxInstallRetries + 1) attempts. Leaving installer in: $downloadDir" "Red"
        throw "FortiClient-VPN-7.2 installation failed"
    }
    
    # Verify software installation
    Write-ColorOutput "Verifying FortiClient-VPN-7.2 installation..." "White"
    
    if ($DisableInstall) {
        Write-ColorOutput "[DRY-RUN] Would verify FortiClient-VPN-7.2 installation" "Cyan"
        ...
        Write-ColorOutput "=== FortiClient-VPN-7.2 Installation Completed Successfully (DRY-RUN) ===" "Green"
        return 0
    }
    else {
        ...
        if ($installationVerified) {
            Write-ColorOutput "FortiClient-VPN-7.2 installation verification successful" "Green"
            ...
            if (-not $DisableCleanup) {
                ...
                try {
                    ...
                    cmd.exe /c Shutdown /r /d p:0:0 /f /t 300 /c "FortiClient update complete. System will reboot in 5 minutes."
                    Write-ColorOutput "Download directory cleaned up successfully" "Green"
                }
                catch {
                    Write-ColorOutput "Warning: Could not clean up download directory: $($_.Exception.Message)" "Yellow"
            ...
            Write-ColorOutput "=== FortiClient-VPN-7.2 Installation Completed Successfully ===" "Green"
            return 0
        }
        else {
            Write-ColorOutput "FortiClient-VPN-7.2 installation verification failed" "Red"
            Write-ColorOutput "Leaving installer in: $downloadDir" "Yellow"
            throw "FortiClient-VPN-7.2 installation verification failed"
        }
    }
}
catch {
    Write-ColorOutput "=== FortiClient-VPN-7.2 Installation Failed ===" "Red"
